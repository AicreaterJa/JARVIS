...
class JarvisSingularity:
    ...
    def _classify_intent(self, query: str) -> bool:
        """Determines if the AI needs to check the live internet."""
        prompt = f"""
        ...
        try:
            ...
            return True # Fallback: always search if unsure

    def _fetch_web_data(self, query: str) -> str:
        """Firewall Bypass: Uses RSS and API disguised as a normal browser."""
        headers = {
    ...
    def generate_response(self, query: str, history: list) -> str:
        ...
        # Inject short-term memory (last 6 interactions)
        ...
        try:
            ...
            return f"Sir, I encountered a critical system error: {str(e)}"

            # ==========================================
# ⚙️ INTERFACE: J.A.R.V.I.S. TERMINAL
# ==========================================
# st.set_page_config MUST be the first Streamlit command, fully flush left.
...
# 🛡️ THE VAULT: Pulling the key securely
API_KEY = st.secrets["GROQ_API_KEY"]
jarvis = JarvisSingularity(api_key=API_KEY)
...
if prompt := st.chat_input("Command the Matrix, Sir..."):
    ...
    with st.chat_message("assistant"):
        answer = jarvis.generate_response(prompt, st.session_state.chat_log)
        st.markdown(answer)
